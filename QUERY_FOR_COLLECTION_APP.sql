...
CREATE TABLE collection_master(
   ...
   CL_STATUS BIT DEFAULT 1 COMMENT "1 IS FOR COLLECTION IS ACTIVE 0 IS FOR COLLECTION CLOSED",
   CL_LAST_PAID_TO INT DEFAULT 0 COMMENT "IT ONLY SHOWS THE LAST COLLECTION DONE BY ADMIN STATUS DEFAULT IS 0",
   CL_CREATED_AT DATE DEFAULT CURRENT_DATE,
   FOREIGN KEY (COL_FOR_CUS_ID) REFERENCES customer_master(CUS_ID),
   FOREIGN KEY (COL_TB_CUS_PL) REFERENCES plan_master(PL_ID) ,
   FOREIGN kEY (CL_LAST_PAID_TO) REFERENCES admin_login(ADMIN_ID));
...
(`COL_FOR_CUS_ID`, `COL_TB_CUS_PL`,`CL_LAST_PAID_TO`) 
