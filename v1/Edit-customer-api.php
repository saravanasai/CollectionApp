...
if($_SERVER['REQUEST_METHOD'] === "POST"){
   ...
   $validation_status=$customer_obj->validate_user_info_feild_on_update($data);
     
   if($validation_status)
   {
      if($customer_obj->check_customer_exist_by_cus_id())
      {
             if($customer_obj->update_customer())
             {
                http_response_code(200);
                echo json_encode(["status"=>"1","data"=>"Customer Information Updated"]);
             }
             else
             {
